 ...
 * <h3>使用示例：</h3>
 * <pre>{@code
 * // 基本用法（使用 ToolRegistry 提供工具名列表）
 * ToolRegistry registry = new ToolRegistry();
 * ErrorHandler handler = new ErrorHandler(
 *     () -> registry.getAllTools().stream().map(Tool::name).collect(Collectors.toList()));
...
public class ErrorHandler {
    ...
    private final Supplier<List<String>> toolNameSupplier;

    /**
     * 创建不带自定义处理器的 ErrorHandler，使用空工具名列表。
     * <p>
     * 此构造函数主要用于向后兼容和测试场景。
     * 生产环境建议使用 {@link #ErrorHandler(Supplier)} 或 {@link #ErrorHandler(Supplier, List)}。
     */
    public ErrorHandler() {
    ...
    /**
     * 创建带工具名供应器的 ErrorHandler。
     * <p>
     * 工具名供应器用于在处理 {@link ToolNotFoundException} 时获取当前可用的工具名列表，
     * 以便通过 Levenshtein 距离算法提供最相似工具名的建议。
     *
     * @param toolNameSupplier 工具名列表供应器，不能为 null
     */
    public ErrorHandler(Supplier<List<String>> toolNameSupplier) {
        this(toolNameSupplier, null);
    }

    /**
     * 创建带工具名供应器和自定义处理器的 ErrorHandler。
     * 自定义处理器在内置处理逻辑之前被调用，如果返回非 null 结果则直接使用。
     *
     * @param toolNameSupplier 工具名列表供应器，不能为 null
     * @param customHandlers   自定义错误处理器列表，可以为 null
     */
    public ErrorHandler(Supplier<List<String>> toolNameSupplier, List<CustomErrorHandler> customHandlers) {
        this.toolNameSupplier = toolNameSupplier != null ? toolNameSupplier : Collections::emptyList;
    ...
    /**
     * 创建带自定义处理器的 ErrorHandler，使用空工具名列表。
     * <p>
     * 此构造函数主要用于向后兼容。
     * 生产环境建议使用 {@link #ErrorHandler(Supplier, List)}。
     *
     * @param customHandlers 自定义错误处理器列表，可以为 null
     */
    public ErrorHandler(List<CustomErrorHandler> customHandlers) {
        this(Collections::emptyList, customHandlers);
    }
    ...
    public ErrorMessage handle(Throwable error) {
        ...
        if (error instanceof LlmApiException) {
            return handleLlmApiError((LlmApiException) error);
        } else if (error instanceof ToolNotFoundException) {
    ...
    /**
     * 处理 LLM API 异常。
     * 从 {@link LlmApiException} 中提取结构化的 {@link LlmErrorInfo}，
     * 构建包含错误类型、用户可读消息和修复建议的 {@link ErrorMessage}。
     *
     * @param error LLM API 异常
     * @return 包含结构化错误信息的错误消息
     */
    public ErrorMessage handleLlmApiError(LlmApiException error) {
        LlmErrorInfo errorInfo = error.getErrorInfo();
        return ErrorMessage.builder()
                .type(errorInfo.getErrorType())
                .message(errorInfo.getMessage())
                .suggestion(errorInfo.getSuggestion())
                .build();
    }
    ...
    public ErrorMessage handleToolNotFound(ToolNotFoundException error) {
        ...
        List<String> availableTools = toolNameSupplier.get();
    ...
    /**
     * 通过 SPI（{@link ServiceLoader}）自动发现并加载 {@link CustomErrorHandler} 实现。
     * <p>
     * 扫描 classpath 中所有通过 {@code META-INF/services/com.wmsay.gpt4_lll.fc.error.CustomErrorHandler}
     * 声明的实现类，逐个添加到自定义处理器列表。单个处理器加载失败时记录警告日志并继续。
     * </p>
     */
    public void loadCustomErrorHandlersFromServiceLoader() {
        try {
            ...
            for (CustomErrorHandler handler : loader) {
                try {
                    ...
                    LOG.fine("SPI registered CustomErrorHandler: " + handler.getClass().getName());
                } catch (Exception e) {
                    LOG.log(Level.WARNING,
                            "Failed to register SPI CustomErrorHandler: " + e.getMessage(), e);
                }
            }
        } catch (Exception e) {
            LOG.log(Level.WARNING, "Failed to load CustomErrorHandler via SPI: " + e.getMessage(), e);
        }
    }
}
