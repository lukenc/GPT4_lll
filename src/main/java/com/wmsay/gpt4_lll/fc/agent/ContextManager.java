...
public class ContextManager {
     ...
     * 构建工具描述文本（需求 19.6）。
     */
    private String buildToolDescriptions(List<McpTool> tools) {
     ...
     */
    private String trimToolDescriptions(List<McpTool> tools, int budgetTokens) {
        if (tools == null || tools.isEmpty()) {
            return "";
        }
        List<McpTool> base = tools.stream()
                .filter(t -> BASE_TOOLS.contains(t.name()))
                .collect(Collectors.toList());
        List<McpTool> others = tools.stream()
                .filter(t -> !BASE_TOOLS.contains(t.name()))
                .collect(Collectors.toList());

        List<McpTool> kept = new ArrayList<>(base);
        int currentTokens = estimateTokenCount(buildToolDescriptions(kept));

        for (McpTool tool : others) {
