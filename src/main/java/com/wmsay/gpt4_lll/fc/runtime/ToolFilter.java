...
public class ToolFilter {
     ...
     */
    public List<Tool> filter(IntentResult intent, List<Tool> allTools) {
     ...
     */
    public List<Tool> filterForStep(PlanStep step, List<Tool> allTools,
                                    LlmCaller llmCaller) {
        if (allTools == null || allTools.isEmpty()) {
            return Collections.emptyList();
        }
        ...
        return Collections.unmodifiableList(new ArrayList<>(allTools));
    }
}
