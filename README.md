...
GPT4_lll是为IntelliJ IDEA用户设计的插件，提供实时代码查询、分析和AI辅助开发服务。借助新的Function Calling引擎，插件现在可以自主使用工具读取、搜索和修改项目文件，成为IDE内的强大AI编程助手。
...
### MCP Tool System
Model Context Protocol（MCP）工具系统，提供文件读取、文件写入、关键词搜索和项目目录树工具。文件写入工具支持四种模式：覆盖写入、补丁替换（str_replace）、追加写入和按行插入，采用原子写入确保数据安全，自动保留原始换行格式。工具沙箱化限制在项目工作区内确保安全。
- 文件写入工具支持四种模式：overwrite、patch、append和insert_after_line
- 原子写入确保数据安全
- 工具沙箱化限制在项目工作区
